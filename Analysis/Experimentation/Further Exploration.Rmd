...
forecast::Acf(ts(na.omit(avg_steer_prices$seas_adj)))


plot(diff(avg_steer_prices$seas_adj, differences = 1))
adf.test(na.omit(diff(avg_steer_prices$seas_adj, differences = 3)), alternative = "stationary")




avg_steer_prices <- avg_steer_prices %>% 
  mutate(diff(avg_steer_prices$seas_adj, differences = 1))
