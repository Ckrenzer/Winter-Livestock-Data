...
# General Packages
pacman::p_load(dplyr, ggplot2, lubridate, readr, shiny, patchwork, clock)
...
-   Fit and evaluate the model. __(order)__
...
dplyr::select(avg_steer_prices, Date, median_price) %>% 
  mutate(`Lo 80` = NA, `Hi 80` = NA, `Lo 95` = NA, `Hi 95` = NA) %>% 
  rbind(dplyr::select(steer_predictions, Date, "median_price" = `Point Forecast`, `Lo 80`, `Hi 80`, `Lo 95`, `Hi 95`))
...
avg_steer_prices %>% 
  ggplot() +
  geom_line(mapping = aes(x = Date, y = median_price)) +
  geom_area(mapping = aes(x = Date, ymin = steer_predictions$`Lo 80`, ymax = steer_predictions$`Hi 80`))




as_tibble(steer_predictions)
