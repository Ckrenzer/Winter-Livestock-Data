...
-   Choose a model order. __order__
...
cat("The first three differences should be used.")


# Making a new column with containing the values after differencing.
avg_steer_prices <- avg_steer_prices %>% 
  mutate("seas_adj_diff" = c(rep(NA, 3), diff(avg_steer_prices$seas_adj, differences = 3)))
...
# Making a new column with containing the values after differencing.
avg_heifer_prices <- avg_heifer_prices %>% 
  mutate("seas_adj_diff" = c(rep(NA, 2), diff(avg_heifer_prices$seas_adj, differences = 2)))
...
With all data stationary, we can now determine the parameters for the model. First, however, we can use the 
...
fit <- forecast(auto.arima(avg_steer_prices$seas_adj, seasonal = FALSE), h = 26)

tsdisplay(residuals(fit), lag.max=45, main='(1,1,1) Model Residuals') 

plot(forecast(auto.arima(avg_steer_prices$seas_adj, seasonal = FALSE), h = 26))
