...
# Storing the average sale price of steers into a variable
avg_steer_prices <- filter(lajunta, Reprod == "str") %>% 
  group_by(Date) %>% 
  summarize("median_price" = median(Price))

# Storing the average sale price of heifers into a variable
avg_heifer_prices <- filter(lajunta, Reprod == "hfr") %>% 
  group_by(Date) %>% 
  summarize("median_price" = median(Price))

# Storing the average sale price of cows into a variable
avg_cow_prices <- filter(lajunta, Reprod == "cow") %>% 
  group_by(Date) %>% 
  summarize("median_price" = median(Price))

# Storing the average sale price of bulls into a variable
...
avg_steer_prices$median_price <- tsclean(ts(avg_steer_prices$median_price))
avg_heifer_prices$median_price <- tsclean(ts(avg_heifer_prices$median_price))
avg_cow_prices$median_price <- tsclean(ts(avg_cow_prices$median_price))
avg_bull_prices$median_price <- tsclean(ts(avg_bull_prices$median_price))
