...
-   Store subsets of the data (steers, heifers, etc.) into their own data frames and calculate the median price at each sale.

-   Make the price a time series column.

-   Calculate moving averages for the data--eight weeks and 26 weeks (6 months) are used for this.

-   Calculate a new column for a seasonally adjusted variable.

-   Test for stationarity with the Augmented Dickey-Fuller test.

-   Correct for non-stationarity via differencing. Use Autocorrelation function (ACF) plots to determine the order of differencing for multiplicative models and partial autocorrelation plots (PACF) to determine the order of differencing for additive models.
...
#estimate of
Acf(ts(na.omit(avg_steer_prices$seas_adj)))
...
ggplot(avg_steer_prices) + 
  ...
  geom_line(aes(x = Daote, y = seas_adj, color = "adj")) +
  ylab('Price') 
