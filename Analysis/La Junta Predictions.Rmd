...
# weight vs price by reproductive status
lajunta %>%
  ggplot() +
...
q1 <- lajunta %>%  
  ...
  group_by(Date) %>% 
  summarize(Price = mean(Price) , Reprod, Quantity) %>% 
  ggplot() +
  geom_point(mapping = aes(x = Date, y = Price, size = Quantity)) +
  geom_line(mapping = aes(x = Date, y = Price)) +
  facet_wrap(~Reprod) +
  ggtitle("Price and Quantity Over Time")
