...
I will be using the tidymodels package to split the data into testing and training sets. The class package will be used for the KNN algorithm. With this algorithm, I will be classifying cattle reproductive status using weight and market price.

```{r import}
if(!require(pacman)) install.packages("pacman")
pacman::p_load(dplyr, readr, tidymodels, class)
...
set.seed(2021)
lajunta_split <- initial_split(lajunta,
                               prob = 0.80,
                               strata = Reprod)
...
lajunta_train <- training(lajunta_split)
lajunta_test <- testing(lajunta_split)
...
The ideal value was either 30 or 31--very close to 29! Using just two variables, we can predict the reproductive status of livestock with 87.5% accuracy! As previously stated, most of these inaccuracies are between steers and heifers. If we are only interested in cows and bulls, we will virtually _always_ have the right prediction!
