...
###### The write-up uses data as of 2/6/2021. The code will still work, though the output numbers I provide may be different.
...
The optimal probability as of the time writing is *0.5399724*. With this new threshold (the old threshold was 0.5, remember), any livestock with a probability of being a steer at or above 0.5399724 will be classified as a steer, with the rest being classified as heifers.
...
knitr::kable(InformationValue::confusionMatrix(lajunta_logit1_test$Reprod, logit1_test_probs, ))
...
knitr::kable(InformationValue::confusionMatrix(lajunta_logit2_test$Reprod, logit2_test_probs))
...
knitr::kable(InformationValue::confusionMatrix(lajunta_logit3_test$Reprod, logit3_test_probs))
...
```
The data were classified incorrectly on the test data `cat(InformationValue::misClassError(lajunta_logit1_test$Reprod, logit1_test_probs, threshold = logit1_optimal))` of the time.
...
The second curve? That's just a gigantic blue rectangle. The same is true of the ROC curve for the third regression. My intution at the outset of this section was correct. There was no error in the third regression.
