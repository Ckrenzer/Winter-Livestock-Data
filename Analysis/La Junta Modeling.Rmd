...
And we can confirm that the standard deviation is, indeed, one after transformation:

```{r scale confimation}
knitr::kable(lajunta_lda %>% 
...
Excellent. Now, let's fit the model and see the results:
...
Let's break this output down bit by bit. Usually, it is good to think of LDA as a dimensionality reduction tool in addition to a predictive one.

#### Prior Probabilities of Groups

These represent the proportions of each reproductive status of the livestock in the training set. Bulls, for example, make up 2.3% of the observations in the training dataset.
...
LD1 = (`r lajunta_lda_fit$scaling[1, 1]`)(Price) `r ifelse(lajunta_lda_fit$scaling[2, 1] > 0, "+", "-")` (`r abs(lajunta_lda_fit$scaling[2, 1])`)(Weight) `r ifelse(lajunta_lda_fit$scaling[3, 1] > 0, "+", "-")` (`r abs(lajunta_lda_fit$scaling[3, 1])`)(Quantity)

#### Proportion of Trace

Displays the percentage separation achieved by each linear discriminant function.
...
The question you are likely most interested wondering, "Does LDA perform better than KNN or any of the logistic regressions," can be found by comparing the classifications of the predictions to the actual observations in the test set.
...
# In other words, sum(TRUE, TRUE, FALSE) equals 2.
mean(lajunta_lda_probs$class == lajunta_lda_test$Reprod)
```

Drum roll please! The model correctly identified the reproductive status `r mean(lajunta_lda_probs$class == lajunta_lda_test$Reprod)` percent of the time! `r ifelse(mean(lajunta_lda_probs$class == lajunta_lda_test$Reprod) > acc[which(acc == max(acc))[1]], "LDA performed better than KNN clustering. For future predictions, it's a good thing this method was checked!", "Well, at least now we know that KNN is a better predictor if we want to classify reproductive status in the future!")` Regardless, they are both reasonably close to one another, so KNN is probably a better choice.

Often times, the people we need to convey these insights to have no background in statistics; Occam's Razor means something approaching, "the simpler explanation is usually the most correct explanation." We will have a much easier time explaining KNN than we would describing dimensionality reduction.
