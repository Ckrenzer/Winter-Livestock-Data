...
#### The Hyperplane
The algorithm is supposed to classify observations by creating a hyperplane with the maximum possible margins (think of margins as the "buffer zone" between the points and the plane--we want the largest "buffer zone" possible). Even in the 3D plane, values between steers and heifers overlap often. To remedy this, a kernel function will be used to transform the data into a higher dimension. You can examine the existing overlap yourself using the interactive diagram below.
