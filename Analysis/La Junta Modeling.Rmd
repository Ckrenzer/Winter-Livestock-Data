...
### Data Overlap
The algorithm is supposed to classify observations by creating a hyperplane with the maximum possible margins (in a 2D setting, you can think of a plane as a line and the margin as the "buffer zone" between the points and the line--we want the largest "buffer zone" possible). Even in the 3D plane, values between steers and heifers overlap often. To remedy this, a kernel function will be used to transform the data into a higher dimension. You can examine the existing overlap yourself using the interactive diagram below.
...
### Cleaning
Character variables will be transformed into factors, and the logical column, "is_Steer," will be added to the data frame.

```{r svm cleaning, echo = TRUE}
lajunta_svm <- lajunta %>% 
  mutate(across(where(is.character), as.factor),
...
lajunta_svm_predict <- predict(lajunta_svm_classifier, newdata = lajunta_svm_test %>% 
          select(Weight, Price, Quantity, is_Steer))
...
The radial function is the most accurate, having a misclassification error rate of `r 100 * mean(lajunta_svm_predict5 == lajunta_svm_test$is_Steer)` percent. Not quite the 95 percent we are looking for--and this, too, performs similarly to KNN clustering--so I think we have reached a plateau in predicting power using just three variables.

Before moving on, let's add try some variations on the model with a radial kernel function.
...
```{r svm optimization, echo = FALSE}

# Making the Model ------------------
# radial, with Date
lajunta_svm_classifier6 <- svm(data = lajunta_svm_train,
                               is_Steer ~ Price + Weight + Quantity + I(lubridate::mdy(Date)), 
                               type = "C-classification", kernel = "radial")
...
lajunta_svm_classifier8 <- svm(data = lajunta_svm_train,
                               is_Steer ~ Price + Weight + Quantity + I(lubridate::mdy(Date)) + `Type`, 
                               type = "C-classification", kernel = "radial")
...
lajunta_svm_predict6 <- predict(lajunta_svm_classifier6, newdata = lajunta_svm_test %>% 
          select(Weight, Date, Price, Buyer, Quantity, is_Steer))

# radial, with Type
lajunta_svm_predict7 <- predict(lajunta_svm_classifier7, newdata = lajunta_svm_test %>% 
          select(Weight, Type, Price, Buyer, Quantity, is_Steer))

# radial, more vars
lajunta_svm_predict8 <- predict(lajunta_svm_classifier8, newdata = lajunta_svm_test %>% 
          select(Weight, Date, Type, Price, Buyer, Quantity, is_Steer))



# Confusion Matrices ---------------
# radial, with Date
knitr::kable(InformationValue::confusionMatrix(lajunta_svm_predict6, lajunta_svm_test$is_Steer), caption = "Radial Kernel Function (Date)")

# radial, with Type
knitr::kable(InformationValue::confusionMatrix(lajunta_svm_predict7, lajunta_svm_test$is_Steer), caption = "Radial Kernel Function (Type)")

# radial, with Date and Type
knitr::kable(InformationValue::confusionMatrix(lajunta_svm_predict8, lajunta_svm_test$is_Steer), caption = "Radial Kernel Function (Date and Type)")
```
...
The accuracy of the model that added only Date is `r 100 * mean(lajunta_svm_predict6 == lajunta_svm_test$is_Steer)` percent, which is a considerable improvement over its counterpart with only the weight, price, and quantity. This is the most accurate model!
