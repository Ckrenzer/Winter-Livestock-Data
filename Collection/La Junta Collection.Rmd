...
I figure it's time to dig a bit deeper into Winter Livestock's market reports. We check all La Junta, CO market reports on the website to find historical data. This implementation assumes a few things about the structure of the text, so there may be small issues here and there with regards to data quality. Many of these problems have been weeded out over time.
...
After cleaning, we do some EDA to find any issues we missed. This is where that copy of the csv file comes in handy! If all that works out, the csv file is ready for use!
...
The collection() function follows these steps:

-   Loop through a set number of Winter Livestock __urls__ and extract the text of the section of the document containing the data.

-   Determine the __delimiter__ to split the text into a list. _If neither delimiter works, skip to the next iteration of the loop._

-   Determine whether the market report is a __La Junta market report__ by searching for 'la junta co' in the text. _If the text is an internet video auction for la junta, it will be ignored. Non La Junta market reports are ignored. Both of these cases skip to the next iteration of the loop._

-   Determine the __date__ of the market report. There are four cases for this, and the last case just adds one week to the previous sale in case the other methods failed. Check each one to understand how the date is determined.

-   Determine which entries have the __sales data__. In the code, this is marked off under the "Header Removal with Keywords" and "Removing Unwanted Sections" sections.

-   Identify the __buyer's name__, and add it to the sales that buyer made, but a name was not provided.

-   Make a __data frame__ and separate the values into different columns.

-   Write to a __csv__. We may save a 'before cleaning' copy for testing the cleaning script.
...
This function is a wrapper around the web scraper. Calls to str_view() are unnecessary and may even slow down the program, but I'm keeping them in to simplify debugging.

```{r collection(), eval = TRUE}

collection <- function(urls){
  
  # Stores previously used URLs in a vector (urls used before running the collection() function)
  ...
  for(URL in urls){
    ...
    # If the url has been used before the execution of the collection() function,
...
I have been changing the URLs around to find different data, and the market reports seem to start around ID == 6700. The max URL at the time of writing is around 14000. The data begins in 2016. In a few instances, the same market report may have two separate market IDs, so a few hundred observations could be replicas.
...
urls <- "http://www.winterlivestock.com/lajunta.php?reportID=13713#marketreport"
