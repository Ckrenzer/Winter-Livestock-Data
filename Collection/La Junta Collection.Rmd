...
-   Check whether you still need the remove.file() call in this document.

-   Make the discussions in "La Junta Collections.Rmd" more flexible. The logistic regression section does not hold up well at all. A part of me debates whether this is necessary...didn't you already provide the disclaimer at the outset? If they want the script to make sense, they can use the old dataset.
...
-   Simplify the cleaning section--it's a hot mess!
...
-   Determine whether the market report is a La Junta market report by searching for 'la junta co' in the text. _If the text is an internet video auction for la junta, it will be ignored. Non La Junta market reports are ignored. Both of these cases skip to the next iteration of the loop._
...
lajunta <- lajunta %>% 
  mutate(Reprod = str_extract(lajunta$Type, "hfr$|str$|bull$|cow$|heifer$|steer$|pair$|bow$|hrf$|hr$"))


# The minor Reprod edits--I don't know what 'sim' is, so I will let KNN decide
lajunta %>% 
  dplyr::select(Reprod) %>% 
  unlist() %>% 
