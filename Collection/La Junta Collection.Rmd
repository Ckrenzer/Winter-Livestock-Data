...
-   Look into why you have some missing values for the buyer name.
...
collection <- function(urls){
  ...
  for(URL in urls){
    ...
    month_of_sale <- str_extract(date_of_sale, "january|february|march|april|\bmay\b|june|july|august|september|october|november|december|\bjan\b|\bfeb\b|\bmar\b|\bapr\b|\bjun\b|\bjul\b|\baug\b|\bsept\b|\boct\b|\bnov\b|\bdec\b")
    ...
    if(is.na(date_of_sale)){
      ...
      y <- clock::get_year(previous_date_of_sale)  
    ...
    if(is.na(date_of_sale)){
      date_of_sale <- as.character(clock::add_weeks(previous_date_of_sale, n = 1))
    ...
    previous_date_of_sale <- lubridate::mdy(date_of_sale)
