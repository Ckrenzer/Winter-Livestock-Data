...
I have been changing the URLs around to find different data, and the market reports seem to start around ID == 6700. The max URL at the time of writing is around 13500. The data begins in 2016. I think some of the market reports are under two separate market IDs, so a few hundred observations may be replicas.

```{r urls-execute}

# This just parses together different market report IDs
...
lajunta$Type <- lajunta %>% 
  ...
  str_replace_all("red\\s+(char)", "\\1") %>% 
  str_replace_all("red\\s+black\\s+(ang)", "\\1") %>% #(removing combinations of colors)
  str_replace_all("red\\s+black\\s+(lim)", "\\1") %>% 
  str_replace_all("red\\s+black\\s+(gel)", "\\1") %>% 
  str_replace_all("red\\s+black\\s+(here)", "\\1") %>% 
  str_replace_all("red\\s+black\\s+(char)", "\\1") %>% 
  str_replace_all("black\\s+red\\s+(ang)", "\\1") %>%
  str_replace_all("black\\s+red\\s+(lim)", "\\1") %>% 
  str_replace_all("black\\s+red\\s+(gel)", "\\1") %>% 
  str_replace_all("black\\s+red\\s+(here)", "\\1") %>%
  str_replace_all("black\\s+red\\s+(char)", "\\1") %>%# removing color ends on this line
  str_squish() %>% 
  str_trim()
...
lajunta$Type <- lajunta %>% 
  dplyr::select(Type) %>% 
  unlist() %>% 
  str_replace_all("black red|black|red|gray|grey|balck|irishred|spot|brwn|", "clr") %>%   # "black red," "black," and "red" are all 
...
# This correction of the Type column is done after finding the errors explicitly. I am 'hard-coding' in the correct value.
lajunta$Type <- lajunta %>% 
  dplyr::select(Type) %>% 
  unlist() %>% 
  str_replace_all("clrclr", "clr") %>% #One of the errors I found
  str_replace_all("clr, clr, char", "char") %>% # a surprisingly common error
  str_replace_all("clrang", "ang") %>% 
  str_replace_all()
